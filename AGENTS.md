...
- `ZOTERO_MCP_GRAPH_MATERIALIZATION_LIMIT` — optional maximum combined persisted records loaded into the in-memory NetworkX graph (default `100000`). Counts papers, citations, topics, authors, and paper-author links; oversized graphs are refused before loading.
...
- **Fulltext index thread-safety**: `_build_fulltext_index()` extracts PDF text in a `ThreadPoolExecutor` but writes to SQLite on the main thread only (sqlite3 connections are not thread-safe). It bounds in-flight futures to `2 * max_workers` and commits successful text in 100-record `store.batch()` transactions.
...
- **Graph materialization ceiling**: `KnowledgeGraph.build_from_store()` checks `GraphStore.get_materialization_counts()` before loading and refuses combined persisted records above `ZOTERO_MCP_GRAPH_MATERIALIZATION_LIMIT` (default 100,000). It fetches paper-author links once per rebuild and reuses that snapshot for both indexes.
- **Entity write batching**: `store_entities` validates first, then batches all entity/link writes in one transaction. `upsert_entity_with_status()` performs insert plus ID lookup transactionally and replaces the old `entity_exists()` preflight without changing the tool response shape.
